...
from ansible_vault._vendored.ansible import constants as C
from ansible_vault._vendored.ansible.errors import AnsibleAssertionError, AnsibleError
from ansible_vault._vendored.ansible.module_utils._text import to_bytes, to_native, to_text
from ansible_vault._vendored.ansible.utils.display import Display
from ansible_vault._vendored.ansible.utils.path import makedirs_safe
...
class FileVaultSecret(VaultSecret):
    ...
    def _read_file(self, filename):
        ...
        verify_secret_is_not_empty(
            vault_pass, msg="Invalid vault password was provided from file (%s)" % filename
        )
...
class ClientScriptVaultSecret(ScriptVaultSecret):
    ...
    def __init__(self, filename=None, encoding=None, loader=None, vault_id=None):
        ...
        display.vvvv(
            "Executing vault password client script: %s --vault-id %s" % (filename, vault_id)
        )
    ...
    def decrypt_and_get_vault_id(self, vaulttext, filename=None):
        ...
        for vault_secret_id, vault_secret in matched_secrets:
            display.vvvvv(
                ...
                % (vault_secret, vault_secret_id, to_text(filename))
            )
            ...
            try:
                ...
                if b_plaintext is not None:
                    ...
                    display.vvvvv(
                        ...
                        % (to_text(file_slug), vault_secret, vault_secret_id)
                    )
                    break
